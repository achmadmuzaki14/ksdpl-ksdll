...
type CoopRow = {
  ...
  partner_name: string;
  type: string;
  maturity_score: number | null; // skala 0-100 (avg dim2-4)
  category: "Early Stage" | "Emerging" | "Developed" | "Advanced" | null;
};
...
type Props = {
  ...
  analytics: {
    // nilai konversi dimensi (skala 0-100)
    dim1: number | null;
    dim2: number | null;
    dim3: number | null;
    dim4: number | null;

    // overall sesuai dokumen: (D1 + D2 + D3 + D4) / 4
    overall_score: number | null;
    overall_category: "Early Stage" | "Emerging" | "Developed" | "Advanced" | null;

    cooperations_count: number;

    // opsional (kalau kamu kirim dari controller)
...
export default function AssessmentShow({
  ...
  const dist = analytics.cooperation_distribution ?? {
    "Early Stage": 0,
    Emerging: 0,
    Developed: 0,
    Advanced: 0,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-6xl space-y-10 px-6 py-8">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-gray-500">
              Kategori mengikuti ambang dokumen: Early Stage / Emerging / Developed / Advanced.
            </p>
          </div>

          <div className="mt-5 space-y-3">
            {(["Early Stage", "Emerging", "Developed", "Advanced"] as const).map((label) => {
          ...
          {totalDist === 0 && (
            <div className="mt-4 rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
              Belum ada cooperation atau belum ada skor (dimensi 2–4 belum terisi).
            </div>
          )}
        </div>
        ...
        <div className="text-xs text-gray-500">
          Catatan: Overall score mengikuti rumus dokumen: (Nilai Konversi Dimensi 1 + 2 + 3 + 4) / 4.
          Jika ada dimensi yang belum memiliki nilai, overall akan tampil “—”.
        </div>
      </div>
    </div>
  );
